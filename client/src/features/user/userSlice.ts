...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setAllUsers: function (state, { payload }) {
      ...
      state.paginationUser.totalPage = Math.ceil(
        payload.data.length / state.paginationUser.pageSize
      )
    ...
    removeUser: function (state, { payload }) {
      ...
      state.paginationUser.totalPage = Math.ceil(
        state.allUser.length / state.paginationUser.pageSize
      )
      const startIndex =
        (state.paginationUser.currentPage - 1) * state.paginationUser.pageSize
    ...
    setSearch: (state, { payload }) => {
      ...
      state.paginationUser.totalPage = Math.ceil(
        userSearch.length / state.paginationUser.pageSize
      )
